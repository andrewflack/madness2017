...
# split play-in teams into 2 entries each (for a total of 68 teams)
...
crowd_picks[,3:8] <- as.data.frame(apply(crowd_picks[,3:8], 2, function(y) y/100))

write.csv(crowd_picks, "data/crowd.csv", row.names = FALSE)
...
# crowd_picks[crowd_picks$team == "Southern California", "team"] <- "Southern Cal"
